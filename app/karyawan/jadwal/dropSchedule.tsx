...
const DropSchedule = (myProp: props) => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        try {
            ...
            if (response.data.success == true) {
                toast(message, {
                    containerId: `toastDrop-${myProp.jadwal.id}`,
                    type: "success"
                })
                setShow(false)
                // reaload page
    ...
    return(
        <div>
            <ToastContainer containerId={`toastDrop-${myProp.jadwal.id}`} />
            <button type="button"
                onClick={() => openModal()}
                className="px-4 py-1 bg-red-600 rounded-md hover:bg-red-500 text-white">
